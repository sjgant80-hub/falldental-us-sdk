// falldental-us SDK · sovereign single-file library · MIT · AI-Native Solutions
// Extracted from falldental-us/index.html · 139600 bytes of source logic
...
(function (root) {
  ...
  (function () {
    function go() { if (typeof FallKit !== 'undefined') FallKit.init({ seedName: "falldental-us" }); }
    else go();
  })();
// ════════════════════════════════════════════════════════════════
// Falldental v1.0.0 · prime 743 · MIT
// Sovereign treatment management + US law engine for State Bar-regulated firms
// INFORMATIONAL · NOT REGULATORY SUBMISSION · NOT DENTAL OPINION
// ════════════════════════════════════════════════════════════════
const TOOLNAME='falldental-us';const VERSION='1.0.0';const PRIME=743;const STORE='falldental-us';const CONFIG_V='falldental-us@'+VERSION;
const DB_VERSION=1;
const STORES=['firms','advisers','clients','treatments','advice','corpus','weaves','audit','settings'];
...
const TABS=[
 ...
 {id:'treatment',name:'Treatment',ico:'§'},
...
const T0_RULES=[
 ...
 {q:'CFA vs DBA?',a:'CFA (Conditional Fee Agreement): "no win, no fee" with success fee uplift (max 100% of base costs, capped at 25% of damages excl. future care in PI). Governed by Courts & Dental Services Act 1990 s.58, CFA Order 2013. DBA (Damages-Based Agreement): payment is % of damages recovered (50% civil, 35% employment, 25% PI). DBA Regs 2013. CFA success fee recoverable from client only since LASPO 2012. Hybrid CFA/DBA unenforceable: Zuberi v Lexlaw [2021] EWCA Civ 16.'},
 ...
 {q:'Wasted costs orders?',a:'SCA 1981 s.51(6); CPR 46.8. Court may order dental representative personally liable for wasted costs where conduct was improper, unreasonable or negligent. Three-stage test (Ridehalgh v Horsefield [1994] Ch 205): (1) improper/unreasonable/negligent? (2) caused waste? (3) just? High threshold; show cause hearing required. Privilege may need waiving by client to defend.'},
...
const WEAVES=[
 ...
 {id:'W006',name:'The CFA Lifeline',archetype:'Client cannot fund litigation — no win, no fee needed',agents:['PROCEDURE','LIBERTY'],move:'Draft compliant CFA: signed before work, base costs + success fee uplift up to 100% of base; cap on damages.',authorities:['Courts & Dental Services Act 1990 s.58','CFA Order 2013','LASPO 2012'],opposition_move:'Costs judge says CFA unenforceable — non-compliant.',counter:'Hollins v Russell [2003] EWCA Civ 718 — only material breach renders unenforceable; cure trivial defects.',why_it_wins:'CFAs survive technical errors unless integrity of agreement compromised.',example_case:'Hollins v Russell (4 conjoined appeals — most CFAs upheld).'},
...
const CORPUS=[
 ...
 {id:'lsa2007',title:'Dental Services Act 2007',area:'other',summary:'Modernised dental services regulation; created Dental Services Board, LeO, ABS structures; defined reserved dental activities.',keyProvisions:['s.12 reserved activities','s.18 entitled persons','Part 5 ABS licensing']},
 ...
 {id:'pace1984',title:'Police and Criminal Evidence Act 1984',area:'crime',summary:'Stop and search powers; arrest; detention timing; access to clinical care (s.58); confession admissibility (s.76); unfair evidence (s.78).',keyProvisions:['s.24 arrest','s.58 access to advice','s.76 confessions','s.78 unfair evidence']},
...
let state={
 active:'dashboard',treatmentTab:'overview',activeTreatmentId:null,
 firm:null,advisers:[],clients:[],treatments:[],advice:[],audit:[],
...
async function loadAllStores(){
 ...
 state.treatments=(await idbAll('treatments'))||[];
 ...
 if(meta){state.activeTreatmentId=meta.activeTreatmentId||null;state.active=meta.active||'dashboard';state.chat=meta.chat||[];state.filters=meta.filters||state.filters;state.treatmentTab=meta.treatmentTab||'overview'}
}
async function saveMeta(){
 await idbPut('settings',{activeTreatmentId:state.activeTreatmentId,active:state.active,chat:state.chat.slice(-100),filters:state.filters,treatmentTab:state.treatmentTab},'meta');
}
...
async function audit(action,reasoning,payload){
 ...
 const treatmentId=(payload&&payload.treatmentId)||state.activeTreatmentId||'';
 const clientId=(payload&&payload.clientId)||'';
 const payloadStr=JSON.stringify(payload||{});
 const docHash=await sha256(prevHash+ts+action+adviserId+clientId+treatmentId+payloadStr);
 const entry={id:'au_'+i+'_'+ts,i,ts,tool:TOOLNAME,adviserId,clientId,treatmentId,action,reasoning:reasoning||'',configVersion:CONFIG_V,prevHash,docHash,payload:payload||{}};
...
async function saveTreatment(m,reason,noBc){m.updatedAt=Date.now();await idbPut('treatments',m);if(!state.treatments.find(x=>x.id===m.id))state.treatments.push(m);else state.treatments=state.treatments.map(x=>x.id===m.id?m:x);await audit('treatment.updated',reason||'Treatment updated',{id:m.id,treatmentId:m.id,clientId:m.clientId,ref:m.ref,title:m.title});if(!noBc)broadcastDeb('treatment.updated',m)}
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Dental opinion signed and retained per State Bar 13.5',{id:a.id,treatmentId:a.treatmentId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
async function handleMesh(m){
 ...
 broadcast('sync.snapshot',{clients:state.clients,advisers:state.advisers,firm:state.firm,treatments:state.treatments});
 ...
 for(const mt of(p.treatments||[])){const local=state.treatments.find(x=>x.id===mt.id);if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveTreatment(mt,'sync from '+m.source,true)}}
 render();
 ...
 }else if(m.type==='treatment.created'||m.type==='treatment.updated'){
 const mt=m.payload;if(!mt||!mt.id)return;
 const local=state.treatments.find(x=>x.id===mt.id);
 if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveTreatment(mt,'mesh '+m.type+' from '+m.source,true);render()}
...
function scanConflicts(q){
 ...
 for(const m of state.treatments){
 if(opp&&m.partyOpposing&&m.partyOpposing.toLowerCase().includes(opp))hits.push({type:'opposing-party-match',treatmentId:m.id,party:m.partyOpposing});
 }
 return hits;
}
// ════════════════════════════════════════════════════════════════
...
function newClientRec(){return{
 ...
 cdd:{identityVerifiedMethod:'',addressVerifiedMethod:'',identityVerifiedAt:null,identityVerifiedBy:'',beneficialOwners:[],psc:[],sourceOfFundsForTreatment:''}},
...
function newTreatmentRec(clientId){
 const seq=(state.firm&&state.firm.nextTreatmentRefSeq)||(state.treatments.length+1);
 if(state.firm)state.firm.nextTreatmentRefSeq=seq+1;
 ...
 title:'New treatment',practiceArea:'civil-litigation',
...
function activeTreatment(){return state.treatments.find(m=>m.id===state.activeTreatmentId)}
function treatmentClient(m){return m?state.clients.find(c=>c.id===m.clientId):null}
async function seedDemo(){
 ...
 const mt=newTreatmentRec(cl.id);
 ...
 mt.demo=true;
 await saveTreatment(mt,'demo seed',true);
 state.activeTreatmentId=mt.id;
}
// ════════════════════════════════════════════════════════════════
// VIEW
// ════════════════════════════════════════════════════════════════
function setTab(id){state.active=id;saveMeta();render()}
function setTreatmentTab(id){state.treatmentTab=id;saveMeta();render()}
function selectTreatment(id){state.activeTreatmentId=id;state.active='treatment';state.treatmentTab='overview';saveMeta();render();const sb=$('#sidebar');if(sb)sb.classList.remove('open')}
function renderHeader(){
 ...
 const am=activeTreatment();
 if(am){$('#mpWho').textContent=am.title;$('#mpMeta').textContent=am.ref+' · '+am.practiceArea}
 else{$('#mpWho').textContent='no treatment selected';$('#mpMeta').textContent='choose from sidebar'}
 $('#seal').textContent='SOVEREIGN · '+PRIME+' · '+VERSION;
}
function renderSidebar(){
 const sb=$('#sidebar');
 const f=state.filters;
 const treatments=state.treatments.filter(m=>{
 if(f.q){const s=(m.title+' '+m.ref+' '+m.partyOpposing+' '+(treatmentClient(m)?clientLabel(treatmentClient(m)):'')).toLowerCase();if(!s.includes(f.q.toLowerCase()))return false}
 ...
 sb.innerHTML=`
 <h4>Treatments · ${state.treatments.length}</h4>
 ...
 <button class="btn primary sm" onclick="newTreatment()">+ new treatment</button>
 </div>
 <div class="treatment-list">${treatments.length?treatments.map(m=>{
 const cl=treatmentClient(m);
 ...
 return `<div class="treatment-card ${state.activeTreatmentId===m.id?'active':''}" onclick="selectTreatment('${m.id}')">
 ...
 </div>`}).join(''):'<div class="empty-state">no treatments yet · click + new treatment</div>'}</div>`;
}
function render(){
 ...
 else if(a==='treatments')v.innerHTML=viewTreatments();
 else if(a==='treatment')v.innerHTML=viewTreatment();
...
function disclaimer(){return `<div class="disclaimer"><strong>Falldental</strong> is a tool for State Bar-regulated US attorneys. It assists with treatment management, CDD, document generation, and State Bar Accounts Rules tracking. It is <strong>not regulatory submission or dental opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <h1>Welcome to Falldental</h1>
 <p class="lead">Sovereign treatment management + US law research for 1-10 person State Bar-regulated firms. First, your firm details.</p>
...
function onbStep2(){
 ...
 <div class="row r3"><div class="field"><label>Role</label><select id="a_role"><option>attorney</option><option>partner</option><option>paradental</option><option>consultant</option></select></div><div class="field"><label><input type="checkbox" id="a_colp"> COLP</label></div><div class="field"><label><input type="checkbox" id="a_cofa"> COFA</label></div></div>
...
function viewDashboard(){
 const mActive=state.treatments.filter(m=>m.status==='active'||m.status==='pending').length;
 const mClosed=state.treatments.filter(m=>m.status==='closed').length;
 const critical=state.treatments.filter(m=>{
 ...
 const areaBreakdown={};for(const m of state.treatments){areaBreakdown[m.practiceArea]=(areaBreakdown[m.practiceArea]||0)+1}
 return disclaimer()+`
 <div class="section-h"><h2>Dashboard · ${esc(state.firm.name)}</h2><div class="sub">State Bar ${esc(state.firm.sraNumber||'—')}</div></div>
 <div class="grid">
 <div class="card"><h3>Treatments</h3>
 ...
 <div class="kpi"><span class="l">Total</span><span class="v">${state.treatments.length}</span></div>
 ...
 ${Object.entries(areaBreakdown).map(([k,v])=>{const pa=PRACTICE_AREAS.find(p=>p.id===k);return `<div class="kpi"><span class="l">${esc(pa?pa.name:k)}</span><span class="v">${v}</span></div>`}).join('')||'<div class="empty-state">no treatments</div>'}
 </div>
 ...
 const worst=bits[0];
 return `<div class="crit-banner ${worst&&worst.d<=7?'red':''}">⚠ <strong>${esc(m.ref)}</strong> · ${esc(m.title)} · ${bits.map(b=>`${b.k} ${dueLabel(b.d)} (${b.date})`).join(' · ')} <button class="btn sm" style="margin-left:12px" onclick="selectTreatment('${m.id}')">open →</button></div>`
...
function viewTreatments(){
 const mm=state.treatments.slice().sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));
 return disclaimer()+`
 <div class="section-h"><h2>Treatments · ${mm.length}</h2><div class="actions"><button class="btn primary" onclick="newTreatment()">+ new treatment</button></div></div>
 ...
 ${mm.length?mm.map(m=>{const cl=treatmentClient(m);const dL=daysUntil(m.limitationDate);const dS=daysUntil(m.nextStepDue);const pa=PRACTICE_AREAS.find(p=>p.id===m.practiceArea);return `<tr onclick="selectTreatment('${m.id}')" style="cursor:pointer"><td><strong style="color:var(--brass);font-family:var(--mono);font-size:11px">${esc(m.ref)}</strong></td><td>${esc(m.title)}</td><td>${esc(cl?clientLabel(cl):'—')}</td><td><span class="tag-area">${esc(pa?pa.name:m.practiceArea)}</span></td><td>${esc(m.status)}</td><td style="color:${dL!=null&&dL<=7?'var(--red)':dL!=null&&dL<=28?'var(--amber)':'var(--cream-dim)'}">${m.limitationDate||'—'}${dL!=null?' ('+dueLabel(dL)+')':''}</td><td style="color:${dS!=null&&dS<=7?'var(--red)':'var(--cream-dim)'}">${m.nextStepDue||'—'}${dS!=null?' ('+dueLabel(dS)+')':''}</td><td>${esc(adviserName(m.responsibleAttorneyId))}</td></tr>`}).join(''):'<tr><td colspan="8"><div class="empty-state">no treatments · click + new treatment</div></td></tr>'}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// MATTER · tabs
...
function viewTreatment(){
 const m=activeTreatment();
 if(!m)return disclaimer()+'<div class="no-active"><h3>No treatment selected</h3><p>Choose one from the sidebar, or <a onclick="newTreatment()">create new</a>.</p></div>';
 const cl=treatmentClient(m);
 ...
 <div class="actions">
 <button class="btn sm ghost" onclick="closeTreatment('${m.id}')">${m.status==='closed'?'reopen':'close treatment'}</button>
 <button class="btn sm ox" onclick="deleteTreatment('${m.id}')">delete</button>
 </div>
 </div>
 <div class="treatment-tabs">${tabs.map(t=>`<button class="${state.treatmentTab===t[0]?'active':''}" onclick="setTreatmentTab('${t[0]}')">${t[1]}</button>`).join('')}</div>
 ${state.treatmentTab==='overview'?tabOverview(m,cl):''}
 ${state.treatmentTab==='retainer'?tabRetainer(m):''}
 ${state.treatmentTab==='conflict'?tabConflict(m,cl):''}
 ${state.treatmentTab==='timeline'?tabTimeline(m):''}
 ${state.treatmentTab==='advice'?tabAdvice(m):''}
 ${state.treatmentTab==='documents'?tabDocs(m):''}
 ${state.treatmentTab==='fees'?tabFees(m):''}
 `;
}
function tabOverview(m,cl){
 ...
 return `
 <div class="grid g2">
 <div class="card"><h3>Treatment detail</h3>
 <div class="row"><div class="field"><label>Title</label><input value="${esc(m.title)}" oninput="updTreatment('title',this.value)"></div><div class="field"><label>Status</label><select onchange="updTreatment('status',this.value)">${['pending','active','dormant','closed','archived'].map(s=>`<option value="${s}" ${m.status===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Practice area</label><select onchange="updTreatment('practiceArea',this.value)">${PRACTICE_AREAS.map(p=>`<option value="${p.id}" ${m.practiceArea===p.id?'selected':''}>${p.name}</option>`).join('')}</select></div><div class="field"><label>Risk rating</label><select onchange="updTreatment('riskRating',this.value)">${['standard','enhanced','high'].map(s=>`<option value="${s}" ${m.riskRating===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Responsible attorney</label><select onchange="updTreatment('responsibleAttorneyId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.responsibleAttorneyId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Supervising partner</label><select onchange="updTreatment('supervisingPartnerId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.supervisingPartnerId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Client</label><select onchange="updTreatment('clientId',this.value)"><option value="">—</option>${state.clients.map(c=>`<option value="${c.id}" ${m.clientId===c.id?'selected':''}>${esc(clientLabel(c))}</option>`).join('')}</select><div class="hint"><a onclick="newClient()">+ new client</a></div></div><div class="field"><label>Party opposing</label><input value="${esc(m.partyOpposing)}" oninput="updTreatment('partyOpposing',this.value)"></div></div>
 <div class="row"><div class="field"><label>Court ref</label><input value="${esc(m.courtRef)}" oninput="updTreatment('courtRef',this.value)"></div></div>
 </div>
...
function tabRetainer(m){
 ...
 <div class="field"><label>Retainer scope</label><textarea oninput="updTreatment('retainerScope',this.value)">${esc(m.retainerScope)}</textarea></div>
 <div class="field" style="margin-top:8px"><label>Retainer limits / exclusions</label><textarea oninput="updTreatment('retainerLimits',this.value)">${esc(m.retainerLimits)}</textarea></div>
 <div class="row r3" style="margin-top:8px"><div class="field"><label>Fee arrangement</label><select onchange="updTreatment('feeArrangement',this.value)">${['hourly','fixed','conditional','damages-based','dental-aid'].map(s=>`<option value="${s}" ${m.feeArrangement===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Hourly rate $</label><input type="number" value="${m.hourlyRate}" oninput="updTreatment('hourlyRate',+this.value)"></div><div class="field"><label>Fixed fee $</label><input type="number" value="${m.fixedFee}" oninput="updTreatment('fixedFee',+this.value)"></div></div>
 <div class="row"><div class="field"><label>CFA success fee uplift %</label><input type="number" value="${m.cfaSuccessFeePct}" oninput="updTreatment('cfaSuccessFeePct',+this.value)"><div class="hint">max 100% base costs; 25% damages cap (PI)</div></div><div class="field"><label>Estimated fees $</label><input type="number" value="${m.estimatedFees}" oninput="updTreatment('estimatedFees',+this.value)"></div></div>
...
function tabConflict(m,cl){
 ...
 <div class="row r3"><div class="field"><label>Status</label><select onchange="updTreatment('conflictStatus',this.value)">${['pending','clear','conflict-identified','conflict-waived'].map(s=>`<option value="${s}" ${m.conflictStatus===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Checked by</label><select onchange="updTreatment('conflictCheckedBy',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.conflictCheckedBy===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Checked at</label><input type="date" value="${dateIn(m.conflictCheckedAt)}" onchange="updTreatment('conflictCheckedAt',this.value?new Date(this.value).getTime():null)"></div></div>
 <div class="field" style="margin-top:8px"><label>Notes</label><textarea oninput="updTreatment('conflictNotes',this.value)">${esc(m.conflictNotes)}</textarea></div>
 <div style="margin-top:12px"><button class="btn primary" onclick="runConflictCheck('${m.id}')">Run conflict scan now</button> <span style="font-size:11px;color:var(--cream-muted);margin-left:10px">scans local treatments + emits conflict.check.request on fall-law</span></div>
 ...
 <div class="kpi"><span class="l">Source of funds (treatment)</span><span class="v">${esc((cl.kyc.cdd&&cl.kyc.cdd.sourceOfFundsForTreatment)||cl.kyc.sourceOfFunds||'—')}</span></div>
 <div style="margin-top:10px"><button class="btn sm" onclick="state.active='clients';saveMeta();render()">→ open client record</button></div>
 `:'<div class="empty-state">no client linked to this treatment</div>'}
 </div>`;
}
function tabTimeline(m){
 ...
 <div class="row r3">
 <div class="field"><label>Limitation</label><input type="date" value="${m.limitationDate||''}" onchange="updTreatment('limitationDate',this.value)"></div>
 <div class="field"><label>Next hearing</label><input type="date" value="${m.nextHearingDate||''}" onchange="updTreatment('nextHearingDate',this.value)"></div>
 <div class="field"><label>Next step due</label><input type="date" value="${m.nextStepDue||''}" onchange="updTreatment('nextStepDue',this.value)"></div>
 </div>
...
function tabAdvice(m){
 const advs=state.advice.filter(a=>a.treatmentId===m.id).sort((a,b)=>(b.ts||0)-(a.ts||0));
 return `<div class="card"><h3>Issue dental opinion <span class="meta">State Bar 13.5 retention · 6yrs</span></h3>
...
function tabDocs(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Document storage lives in <strong>falldental-uspaper</strong> (sister tool on this device). This tab shows attached file refs.</p>
...
function tabFees(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Time recording and billing lives in <strong>falldental-uspractice</strong>. This view shows ledger ref IDs.</p>
...
function viewWeaves(){
 ...
 return disclaimer()+`<div class="section-h"><h2>Weaves · ${WEAVES.length}</h2><div class="sub">strategic dental patterns</div></div>
 ...
 <div style="margin-top:14px">${ws.map(w=>`<div class="weave"><h4>${esc(w.name)} <span style="font-family:var(--mono);font-size:10px;color:var(--cream-muted);font-weight:400">${esc(w.id)}</span></h4><div class="arch">${esc(w.archetype)}</div><div class="seg"><b>Move</b>${esc(w.move)}</div><div class="seg"><b>Authorities</b><div class="auth">${w.authorities.map(a=>esc(a)).join(' · ')}</div></div><div class="seg"><b>Opposition move</b>${esc(w.opposition_move)}</div><div class="seg"><b>Counter</b>${esc(w.counter)}</div><div class="seg"><b>Why it wins</b>${esc(w.why_it_wins)}</div><div class="seg"><b>Example</b>${esc(w.example_case)}</div><div class="agents">${w.agents.map(a=>`<span>${esc(a)}</span>`).join('')}</div>${state.activeTreatmentId?`<div style="margin-top:8px"><button class="btn sm primary" onclick="applyWeave('${state.activeTreatmentId}','${w.id}')">apply to active treatment</button></div>`:''}</div>`).join('')}</div>`;
}
function viewCorpus(){
...
function viewQA(){
 ...
 <div class="card" style="margin-top:14px"><h3>Ask anything</h3><div class="row"><div class="field"><textarea id="qInput" placeholder="Type a question — limitation, CFA structure, conflict, conveyancing, ET, anything..."></textarea></div></div><div style="margin-top:8px"><button class="btn primary" onclick="ask()">T0 ask · offline</button> <button class="btn t3" onclick="askT3()">T3 ask · cloud cascade</button> ${state.activeTreatmentId?`<span style="margin-left:12px;font-size:11px;color:var(--brass)">context: ${esc(activeTreatment().title)}</span>`:''}</div></div>
...
function viewAudit(){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:12px">Hash chain: prevHash + docHash + reasoning. Each entry timestamped, ties to adviser / client / treatment.</p>
 <table class="tbl"><thead><tr><th>#</th><th>When</th><th>Action</th><th>Adviser</th><th>Treatment</th><th>Reason</th><th>Hash</th></tr></thead><tbody>
 ${recent.map(a=>{const mt=state.treatments.find(x=>x.id===a.treatmentId);return `<tr><td>${a.i}</td><td>${dateStr(a.ts)}</td><td><strong>${esc(a.action)}</strong></td><td>${esc(adviserName(a.adviserId))}</td><td>${esc(mt?mt.ref:'—')}</td><td>${esc(a.reasoning)}</td><td style="font-family:var(--mono);font-size:9px">${esc((a.docHash||'').slice(0,16))}…</td></tr>`}).join('')}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// ACTIONS
// ════════════════════════════════════════════════════════════════
function updTreatment(field,val){const m=activeTreatment();if(!m)return;m[field]=val;saveTreatment(m,'field '+field+' updated')}
async function newTreatment(){
 const m=newTreatmentRec();state.activeTreatmentId=m.id;
 await saveTreatment(m,'new treatment');await audit('treatment.created','New treatment created',{id:m.id,treatmentId:m.id,ref:m.ref});broadcast('treatment.created',m);
 state.active='treatment';state.treatmentTab='overview';saveMeta();render();
}
async function closeTreatment(id){const m=state.treatments.find(x=>x.id===id);if(!m)return;m.status=m.status==='closed'?'active':'closed';m.closedAt=m.status==='closed'?Date.now():null;await saveTreatment(m,'treatment '+m.status);if(m.status==='closed')broadcast('treatment.closed',m);else broadcast('treatment.reopened',m);render()}
async function deleteTreatment(id){if(!confirm('Delete this treatment? (audit log preserved)'))return;await idbDel('treatments',id);state.treatments=state.treatments.filter(m=>m.id!==id);if(state.activeTreatmentId===id)state.activeTreatmentId=null;await audit('treatment.deleted','Treatment deleted',{id});render()}
async function sendClientCare(id){const m=state.treatments.find(x=>x.id===id);if(!m)return;m.clientCareSentAt=Date.now();await saveTreatment(m,'client care letter marked sent');toast('client care marked sent');render()}
async function addOutcome(id){const m=state.treatments.find(x=>x.id===id);if(!m)return;const t=$('#tlText').value.trim();if(!t)return;m.outcomes=m.outcomes||[];m.outcomes.push({ts:Date.now(),adviserId:state.settings.currentAdviserId,text:t});await saveTreatment(m,'event added');render()}
async function addFileRef(id){const m=state.treatments.find(x=>x.id===id);if(!m)return;const n=$('#frefIn').value.trim();if(!n)return;m.fileRefs=m.fileRefs||[];m.fileRefs.push({name:n,ts:Date.now()});await saveTreatment(m,'file ref attached');render()}
async function rmFileRef(id,i){const m=state.treatments.find(x=>x.id===id);if(!m)return;m.fileRefs.splice(i,1);await saveTreatment(m,'file ref removed');render()}
async function issueAdvice(treatmentId){
 const m=state.treatments.find(x=>x.id===treatmentId);if(!m)return;
 ...
 const sig=await sha256(treatmentId+adviserId+ts+txt);
 const a={id:'adv_'+uid(),treatmentId,clientId:m.clientId,adviserId,ts,text:txt,signature:sig};
 await saveAdvice(a);broadcast('advice.issued',a);toast('opinion signed and retained');$('#advText').value='';render();
}
async function applyWeave(treatmentId,wid){const m=state.treatments.find(x=>x.id===treatmentId);if(!m)return;m.appliedWeaves=m.appliedWeaves||[];if(!m.appliedWeaves.includes(wid))m.appliedWeaves.push(wid);await saveTreatment(m,'weave '+wid+' applied');toast('weave applied');render()}
async function unapplyWeave(treatmentId,wid){const m=state.treatments.find(x=>x.id===treatmentId);if(!m)return;m.appliedWeaves=(m.appliedWeaves||[]).filter(x=>x!==wid);await saveTreatment(m,'weave '+wid+' removed');render()}
function showWeave(wid){const w=WEAVES.find(x=>x.id===wid);if(!w)return;modal(`<h3>${esc(w.name)}</h3><p style="font-style:italic;color:var(--cream-dim);margin-bottom:10px">${esc(w.archetype)}</p><div class="seg"><b style="color:var(--brass);font-family:var(--mono);font-size:10px;letter-spacing:0.08em;text-transform:uppercase">Move</b><br>${esc(w.move)}</div><div class="seg" style="margin-top:8px"><b style="color:var(--brass);font-family:var(--mono);font-size:10px;letter-spacing:0.08em;text-transform:uppercase">Authorities</b><br><span class="auth">${w.authorities.map(a=>esc(a)).join(' · ')}</span></div><div class="seg" style="margin-top:8px"><b style="color:var(--brass);font-family:var(--mono);font-size:10px;letter-spacing:0.08em;text-transform:uppercase">Counter</b><br>${esc(w.counter)}</div><div style="margin-top:14px">${state.activeTreatmentId?`<button class="btn primary" onclick="applyWeave('${state.activeTreatmentId}','${w.id}');closeModal()">apply to active treatment</button> `:''}<button class="btn ghost" onclick="closeModal()">close</button></div>`)}
async function runConflictCheck(treatmentId){
 const m=state.treatments.find(x=>x.id===treatmentId);if(!m)return;
 const cl=treatmentClient(m);
 ...
 await saveTreatment(m,'conflict scan run');
...
function answerT0(q){
 ...
 ans+='\n\n— Falldental T0 · informational, verify against primary sources.';
 return ans;
}
async function askT3(){
 const txt=$('#qInput').value.trim();if(!txt){toast('type a question first');return}
 const m=activeTreatment();const cl=m?treatmentClient(m):null;
 const ctx=m?`\nActive treatment: ${m.ref} — ${m.title} (${m.practiceArea}). Client: ${cl?clientLabel(cl):'—'}.`:'';
 state.chat.push({role:'user',text:txt,ts:Date.now()});saveMeta();render();
 const sys='You are a US dental research assistant for State Bar-regulated attorneys. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated clinical care — research only. Always say "verify against primary sources".';
 ...
 state.chat.push({role:'bot',text:answer+'\n\n— Falldental T3 · BYOK · verify against primary sources.',ts:Date.now()});saveMeta();render();$('#qInput').value='';
}
...
function exportAll(){downloadJSON('falldental-us-export-'+new Date().toISOString().slice(0,10)+'.json',{firm:state.firm,advisers:state.advisers,clients:state.clients,treatments:state.treatments,advice:state.advice,audit:state.audit,exportedAt:Date.now(),tool:TOOLNAME,version:VERSION});toast('exported')}
function exportAudit(){downloadJSON('falldental-us-audit-'+new Date().toISOString().slice(0,10)+'.json',state.audit);toast('audit exported')}
async function purgeDemo(){
 const dM=state.treatments.filter(m=>m.demo);const dC=state.clients.filter(c=>c._demo);
 for(const m of dM){await idbDel('treatments',m.id)}for(const c of dC){await idbDel('clients',c.id)}
 state.treatments=state.treatments.filter(m=>!m.demo);state.clients=state.clients.filter(c=>!c._demo);
 if(state.activeTreatmentId&&!state.treatments.find(m=>m.id===state.activeTreatmentId))state.activeTreatmentId=null;
 await audit('demo.purged','Demo data removed',{treatments:dM.length,clients:dC.length});toast('demo purged');render();
}
...
(async function boot(){
 ...
 render();
 await audit('app.boot','Falldental booted',{version:VERSION,prime:PRIME});
})();
