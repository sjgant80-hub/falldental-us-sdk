...
// Extracted from falldental-us/index.html · 139519 bytes of source logic
...
// Sovereign treatment management + UK law engine for SRA-regulated firms
...
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Dental opinion signed and retained per SRA 13.5',{id:a.id,treatmentId:a.treatmentId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
function disclaimer(){return `<div class="disclaimer"><strong>Falldental</strong> is a tool for SRA-regulated UK solicitors. It assists with treatment management, CDD, document generation, and SRA Accounts Rules tracking. It is <strong>not regulatory submission or dental opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <p class="lead">Sovereign treatment management + UK law research for 1-10 person SRA-regulated firms. First, your firm details.</p>
...
function onbStep2(){
 ...
 <div class="row r3"><div class="field"><label>Role</label><select id="a_role"><option>solicitor</option><option>partner</option><option>paradental</option><option>consultant</option></select></div><div class="field"><label><input type="checkbox" id="a_colp"> COLP</label></div><div class="field"><label><input type="checkbox" id="a_cofa"> COFA</label></div></div>
...
function tabAdvice(m){
 ...
 return `<div class="card"><h3>Issue dental opinion <span class="meta">SRA 13.5 retention · 6yrs</span></h3>
...
async function askT3(){
 ...
 const sys='You are a UK dental research assistant for SRA-regulated solicitors. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated clinical care — research only. Always say "verify against primary sources".';
